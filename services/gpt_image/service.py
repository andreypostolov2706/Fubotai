...
from .database import init_db, get_session, ImageGeneration
...
class GPTImageService(BaseService):
    """Сервис генерации изображений GPT-Image 1.5"""
    ...
    async def install(self) -> bool:
        ...
        try:
            init_db()
            logger.info("GPT-Image: База данных инициализирована")
            return True
        except Exception as e:
            logger.error(f"GPT-Image: Ошибка установки: {e}")
            return False
    ...
    def get_user_menu_items(self, user_id: int, user_data=None) -> list[MenuItem]:
        ...
        return [
            MenuItem(
                ...
                order=15,  # После Nano Banano (10), перед Veo (20)
            )
        ]
    ...
    async def handle_callback(
        ...
        try:
            ...
            # Подтверждение генерации
                ...
                return await self._execute_generation(user_id, context.message_id, context.chat_id, mode, context)
            ...
            # Выбор параметра
                ...
                if param == "quality":
                    return Response(
                        text=msg.SELECT_QUALITY.format(current=msg.QUALITY_NAMES.get(state_data.get("quality", "high"), "high")),
                        keyboard=kb.quality_keyboard(state_data.get("quality", "high"), mode)
                    )
                elif param == "size":
                    return Response(
                        text=msg.SELECT_SIZE.format(current=msg.SIZE_NAMES.get(state_data.get("image_size", "1024x1024"), "1024x1024")),
                        keyboard=kb.size_keyboard(state_data.get("image_size", "1024x1024"), mode)
                    )
                elif param == "background":
                    return Response(
                        text=msg.SELECT_BACKGROUND.format(current=msg.BACKGROUND_NAMES.get(state_data.get("background", "auto"), "auto")),
                        keyboard=kb.background_keyboard(state_data.get("background", "auto"), mode)
                    )
                elif param == "format":
                    return Response(
                        text=msg.SELECT_FORMAT.format(current=msg.FORMAT_NAMES.get(state_data.get("output_format", "png"), "png")),
                        keyboard=kb.format_keyboard(state_data.get("output_format", "png"), mode)
                    )
            ...
            # Установка параметра при генерации
                ...
                mode = params.get("1", "generate")
                if mode == "edit":
                    result = await self.edit_handler.update_param(user_id, "quality", value)
                else:
                    result = await self.generate_handler.update_param(user_id, "quality", value)
                return Response(text=result["text"], keyboard=result["keyboard"])
                ...
                mode = params.get("1", "generate")
                if mode == "edit":
                    result = await self.edit_handler.update_param(user_id, "size", value)
                else:
                    result = await self.generate_handler.update_param(user_id, "size", value)
                return Response(text=result["text"], keyboard=result["keyboard"])
                ...
                mode = params.get("1", "generate")
                if mode == "edit":
                    result = await self.edit_handler.update_param(user_id, "background", value)
                else:
                    result = await self.generate_handler.update_param(user_id, "background", value)
                return Response(text=result["text"], keyboard=result["keyboard"])
                ...
                mode = params.get("1", "generate")
                if mode == "edit":
                    result = await self.edit_handler.update_param(user_id, "format", value)
                else:
                    result = await self.generate_handler.update_param(user_id, "format", value)
                return Response(text=result["text"], keyboard=result["keyboard"])
            ...
            # Настройки
                ...
                if sub_action == "quality":
                    result = await self.settings_handler.show_quality_selection(user_id)
                elif sub_action == "size":
                    result = await self.settings_handler.show_size_selection(user_id)
                elif sub_action == "background":
                    result = await self.settings_handler.show_background_selection(user_id)
            ...
            # Установка настроек
                ...
                if setting == "quality":
                    result = await self.settings_handler.set_quality(user_id, value)
                elif setting == "size":
                    result = await self.settings_handler.set_size(user_id, value)
                elif setting == "background":
                    result = await self.settings_handler.set_background(user_id, value)
                elif setting == "format":
            ...
            logger.error(f"GPT-Image callback error: {e}")
    ...
    async def handle_message(
        ...
        try:
            # Ожидание промпта для Text-to-Image
            ...
            # Ожидание изображения для Image-to-Image
            elif state == "waiting_image":
                if message.photo_file_id:
                    # Используем file_id как placeholder для URL
                    image_url = f"tg://file/{message.photo_file_id}"
            ...
            logger.error(f"GPT-Image message error: {e}")
            return Response(text=f"❌ Ошибка: {e}", keyboard=kb.main_menu_keyboard())
    ...
    async def _execute_generation(
        ...
        cost_usd = FalClient.calculate_cost(
            state_data["quality"],
            state_data.get("image_size", "1024x1024")
        )
        ...
        if balance < cost_gton:
            cost_fiat = await self.gton_to_fiat(cost_gton)
            return Response(
        ...
        # Списываем средства
        await self.core.deduct_balance(user_id, cost_gton, f"GPT-Image: {state_data['prompt'][:50]}")
        ...
        try:
            generation = ImageGeneration(
                user_id=user_id,
                mode=state_data.get("mode", "text_to_image"),
        ...
        try:
            ...
            if state_data.get("mode") == "image_to_image":
            ...
            session = get_session()
            try:
                gen = session.query(ImageGeneration).filter(ImageGeneration.id == generation_id).first()
                if gen:
            ...
            text = msg.GENERATION_SUCCESS.format(
                ...
                size=state_data.get("image_size", "1024x1024"),
            ...
            # Возвращаем средства
            await self.core.add_balance(user_id, cost_gton, f"Возврат: GPT-Image ошибка")
    ...
    async def _publish_to_gallery(self, user_id: int, generation_id: int):
        ...
        try:
            ...
            if gen and gen.image_url and not gen.published_to_gallery:
    ...
    async def _show_main_menu(self, user_id: int) -> Response:
        ...
        # Ориентировочная стоимость (low quality 1024x1024)
        config = await self.get_service_config()
        margin = config.get("margin_multiplier", 0.3)
        base_price = 0.009  # low quality 1024x1024
        price_usd = base_price * (1 + margin)
        price_gton = await self.usd_to_gton(price_usd)
        
        text = msg.MAIN_MENU.format(
            balance=f"{balance:.4f}",
            price=f"{float(price_gton):.4f}"
        )

...
class ServiceRegistry:
    ...
    async def enable(self, service_id: str) -> bool:
        ...
        # Load if not loaded (после закрытия сессии)
        if service_id not in self._services and service_record:
            # Перечитываем запись для загрузки
            async with get_db() as session:
                result = await session.execute(
                    select(Service).where(Service.id == service_id)
                )
    ...
    async def disable(self, service_id: str) -> bool:
        ...
        if service_id in self._services:
            del self._services[service_id]
            logger.info(f"Service disabled and unloaded: {service_id}")

...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check if user is in any service state (get the most recently updated one)
    ...
    async with get_db() as session:
        ...
        result = await session.execute(
            ...
            ).order_by(desc(UserService.state_updated_at))
        )
    ...
    if user_service and user_service.service_id == "core":
        ...
        elif state == "sbp_custom_amount":
            from .topup import handle_sbp_custom_input
            await handle_sbp_custom_input(update, context, user_id, lang)
            return
